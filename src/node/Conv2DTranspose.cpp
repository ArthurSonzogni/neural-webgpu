...
#include "node/Conv2DTranspose.wgsl.hpp"
#include "node/NodePipeline.hpp"

Node Conv2DTranspose(Node input, int kernel_size, int channels, int stride) {
  class Impl : public NodeImpl {
   public:
    std::string Name() override { return "Conv2DTranspose"; }
    ...
    Impl(Node input, int kernel_size, int channels, int stride)
      ...
      ASSERT(input_dimensions == 4, "Conv2DTranspose input must be 4D.");
      ...
      const int output_dx = (input_dx - 1) * stride + kernel_size;
      const int output_dy = (input_dy - 1) * stride + kernel_size;
      ...
      outputs[0].SetName("Conv2DTranspose outputs[0]");
      ...
      const float noise = 0.1f / std::sqrt(            //
                                     ...
                                     kernel_size *     //
                                     input_channels);  //
      weights[0].SetName("Conv2DTranspose weights[0]");
      weights[0].FillRandomGaussian(gpu(), 0.f, noise);

      SetupGradients();

      wgpu::ShaderModule module =
          Shader(gpu(), fmt::format(wgsl::Conv2DTranspose,  //
                                    input_dx,               //
                                    input_dy,               //
                                    input_channels,         //
                                    output_channels_,       //
